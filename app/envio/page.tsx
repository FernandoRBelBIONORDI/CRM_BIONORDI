    ...
    <div className="h-full flex flex-col font-sans">
      ...
      <div className="flex justify-between items-center px-4 mb-2">
        ...
        <div className="flex items-center gap-2 text-[12px] font-bold text-gray-400">
          ...
          {selected.size > 0 && <span className="bg-[#EEF3FC] text-[#427DFA] px-3 py-1.5 rounded-full">{selected.size} seleccionados</span>}
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden p-6 pt-4 gap-[24px]">
        ...
        <div className="w-[320px] shrink-0 card flex flex-col overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
            <h3 className="font-bold text-[14px] text-[#202538] flex items-center gap-2">
              <span className="w-5 h-5 bg-[#EEF3FC] text-[#427DFA] rounded-md flex items-center justify-center text-[10px] font-bold">1</span>
              Selección
            </h3>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1.5">
            {leads.map(lead=>(
              <div key={lead.id} onClick={()=>toggle(lead.id)}
                className={`flex items-center gap-3 p-3 cursor-pointer rounded-xl transition-all border ${selected.has(lead.id)?"bg-[#EEF3FC] border-[#427DFA]/20":"border-gray-100 hover:bg-gray-50 hover:border-gray-200"} ${sent[lead.id]?.done?"opacity-60":""}`}>
                ...
                <button onClick={e=>{e.stopPropagation();genScripts(lead);}}
                  className={`w-8 h-8 flex items-center justify-center rounded-full transition-all shrink-0 ${activeLead?.id===lead.id?"bg-[#427DFA] text-white shadow-md":"text-[#427DFA] bg-white border border-blue-100 hover:bg-[#427DFA] hover:text-white"}`}>
        ...
        <div className="flex-1 card flex flex-col overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
            <h3 className="font-bold text-[14px] text-[#202538] flex items-center gap-2">
              <span className="w-5 h-5 bg-[#EEF3FC] text-[#427DFA] rounded-md flex items-center justify-center text-[10px] font-bold">2</span>
              Script IA
              {activeLead && <span className="text-[12px] font-bold text-[#427DFA] bg-[#EEF3FC] px-3 py-1 rounded-full ml-1">{activeLead.nombre}</span>}
            </h3>
            {activeLead && (
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-[#FAFCFF]">
            {loadingScr && (
              <div className="flex flex-col items-center justify-center h-full gap-4 text-[#427DFA]">
            ...
            {!loadingScr && !scripts && (
              <div className="flex flex-col items-center justify-center h-full gap-4 opacity-40">
                <div className="w-16 h-16 rounded-[20px] bg-[#EEF3FC] flex items-center justify-center">
                  <Sparkles size={28} className="text-[#427DFA]" fill="currentColor"/>
                </div>
              ...
              <div key={k} className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden hover:border-blue-200 transition-all group/s">
                <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between bg-[#F8FAFC]">
                  <span className="text-[11px] font-bold text-[#2B355A] uppercase tracking-wider flex items-center gap-2">
                    <Sparkles size={11} className="text-[#427DFA]"/>{SCRL[k]||k}
                  </span>
                  <div className="flex gap-2">
                    <button onClick={()=>setEditing(isEditing?null:k)}
                      className={`flex items-center gap-1.5 text-[11px] font-bold rounded-full px-3 py-1.5 transition-all border ${isEditing?"bg-[#427DFA] text-white border-[#427DFA]":"bg-white border-gray-200 text-gray-500 hover:text-[#427DFA] hover:border-blue-200"}`}>
                      {isEditing ? "Listo" : "Editar"}
                    </button>
                    <button onClick={()=>{navigator.clipboard.writeText(current);setCopied(k);setTimeout(()=>setCopied(null),1500);}}
                      className="flex items-center gap-1.5 text-[11px] font-bold bg-white border border-gray-200 rounded-full px-3 py-1.5 text-gray-500 hover:text-[#427DFA] hover:border-blue-200 transition-all">
                ...
                <div className="p-5">
                  {isEditing
                    ? <textarea value={current} onChange={e=>setEditedScripts(p=>({...p,[k]:e.target.value}))}
                        className="w-full text-[13px] leading-relaxed text-[#374151] font-medium bg-[#F8FAFF] border border-blue-200 rounded-xl p-3 outline-none resize-none focus:border-[#427DFA] transition-colors"
                        rows={6}/>
        ...
        <div className="w-[300px] shrink-0 card flex flex-col overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
            <h3 className="font-bold text-[14px] text-[#202538] flex items-center gap-2">
              <span className="w-5 h-5 bg-[#EEF3FC] text-[#427DFA] rounded-md flex items-center justify-center text-[10px] font-bold">3</span>
              Envío y Cierre
            </h3>
            {selLeads.length > 0 && (
  ...
  return expanded ? (
    <div className="bg-[#F8FAFF] border border-blue-100 rounded-xl p-3 space-y-2">
      ...
      <input type="date" defaultValue={defaultDate} onChange={e=>setDate(e.target.value)}
        className="w-full text-[12px] font-bold text-[#1E293B] bg-white border border-gray-200 rounded-lg px-3 py-1.5 outline-none focus:border-[#427DFA]/40"/>
      <div className="flex gap-1.5">
        <button onClick={()=>onMark(lead, date||defaultDate)}
          className="flex-1 text-[11px] font-bold bg-[#427DFA] text-white py-1.5 rounded-lg hover:bg-[#3668d6] transition-colors flex items-center justify-center gap-1">
    ...
    <button onClick={()=>setExpanded(true)}
      className="w-full text-[11px] font-bold text-[#427DFA] bg-[#EEF3FC] py-1.5 rounded-lg hover:bg-[#427DFA] hover:text-white transition-all flex items-center justify-center gap-1.5">
