...
const VENTA_FEATURES: Record<string, string[]> = {
  ...
  "M5": [
    ...
    "<strong>Alta Portabilidad</strong>: Peso ligero de 4.5 kg y diseño de maletín robusto, ideal para acompañar en rondas hospitalarias intensivas.",
  ...
  "DP-50 Exp": [
    "<strong>Esencial y Confiable</strong>: Consola portátil B/W con pantalla LCD 15\", batería > 2 h. Incluye THI, iBeam™, iClear™ e iScape™.",
...
export default function CotizarVentaPageWrapper() {
  return <Suspense><CotizarVentaPage /></Suspense>;
}

function CotizarVentaPage() {
  ...
  const [features, setFeatures] = useState<string[]>([
    "Excelente equipo médico con altos estándares de calidad y precisión diagnóstica.",
    "",
    "",
    ""
  ]);
  ...
  const getPolGarantia = () => polGarantia.trim() !== "" ? polGarantia : `Garantía directa contra defectos de fábrica, soporte técnico especializado y capacitación operativa.`;
  ...
  useEffect(() => { pdfB64Ref.current = null; },
    [cliNombre, cliContacto, cliTel, cliCorreo, cliDireccion,
      eqTipo, eqMarca, eqModelo, eqSerie, eqDescripcion, items, descuento, conIVA,
  ...
  // Autopopulate features when catalog model changes
  useEffect(() => {
    if (equipoMode === "catalogo" && eqModelo) {
      const foundKey = Object.keys(VENTA_FEATURES).find(k => eqModelo.toLowerCase().includes(k.toLowerCase()));
      if (foundKey) {
        setFeatures(VENTA_FEATURES[foundKey]);
      } else if (eqDescripcion) {
        setFeatures(eqDescripcion.split('\n').map(s => s.trim()).filter(Boolean).slice(0, 4));
      } else {
        setFeatures(["Excelente equipo médico con altos estándares de calidad y precisión diagnóstica.", "", "", ""]);
      }
    }
  }, [eqModelo, equipoMode, eqDescripcion]);
    ...
    if (qId) {
      ...
      fetch(`/api/cotizaciones/${qId}`).then(r => r.json()).then(d => {
        ...
        if (c) {
          ...
          setEqDescripcion(c.eq_descripcion || "");
          setNotas(c.notas || "");
      ...
      fetch("/api/cotizaciones?nextfolio=1&tipo=venta")
  ...
  const seleccionarDelCatalogo = async (eq: CatalogoItem) => {
    ...
    if (eq.fotos_json) {
      try {
        ...
        if (parsed.length > 0) setEqFotos(parsed);
      } catch {}
    }
    if (eq.imagen_path) {
  ...
  const generarParrafoAlcance = (): string => {
    ...
    return `La presente propuesta comercial contempla el suministro de: ${lista}. ${cleanFeatures} El equipo incluye garantía directa contra defectos de fábrica, soporte técnico especializado y capacitación operativa para asegurar su óptimo aprovechamiento en el entorno clínico.`;
  };
  ...
  const buildPDFHtml = async (): Promise<{ html: string; folio: string }> => {
    ...
    if (!usedFolio) {
      const r = await fetch("/api/cotizaciones?nextfolio=1&tipo=venta");
      const d = await r.json();
      usedFolio = d.folio as string;
      setFolio(usedFolio);
    }

    const propText = propuestaPDF.trim() || generarParrafoAlcance() || "Propuesta comercial para el suministro de equipos médicos con garantía y soporte técnico.";
    ...
    const equipoHTML = (eqMarca || eqModelo || eqTipo) ? `
    <div class="eq-card">
    ...
    <div class="tech-card avoid-break">
      ...
      <div style="display:flex;gap:20px;align-items:flex-start;margin-top:8px;">
        ${imgTransductor ? `<div style="flex:0 0 280px;"><div class="img-container" style="height:190px;padding:10px;border:1px solid #CBD5E1;border-radius:8px;background:#fff;display:flex;align-items:center;justify-content:center;"><img src="${imgTransductor}" alt="Foto Principal" style="max-width:100%;max-height:170px;width:auto;height:auto;background:white;" /></div></div>` : ""}
...
<div class="tech-card avoid-break" style="margin-bottom:20px;border-left:4px solid #4E60A9;margin-top:0px;">
  ...
  <div style="background:#EEF0F7;border:1px solid #C5CAE0;border-radius:10px;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;">
    <div>
      ...
      <div style="font-size:11px;color:#4E60A9;">Incluye garantía directa y soporte técnico especializado</div>
    </div>
...
<div class="signatures-wrapper">
  ...
  const persistToDB = async (usedFolio: string) => {
    const body = {
      lead_id: leadId || null,
      tipo: "venta", folio: usedFolio, monto_total: total,
      items_json: {
  ...
  const enviarEmail = async () => {
    ...
    try {
      ...
      // En Venta usamos el API general /api/email que sabemos funciona perfecto
...
<body style="margin:0;padding:0;background:#ffffff;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:20px;border:1px solid #E2E8F0;">
    <h2 style="color:#4E60A9;">Propuesta Comercial ${f}</h2>
    <p>Estimado cliente,</p>
    <p>Le hacemos llegar la propuesta comercial correspondiente al suministro de equipo médico.</p>
    ...
    <p>Atentamente,<br/><strong>${sigName}</strong><br/>${sigRole}<br/>Bionordi</p>
      ...
      const res = await fetch("/api/email", {
        ...
        body: JSON.stringify({
          to: emailTo.trim(),
          subject: `Cotización ${f} — Propuesta Comercial · Bionordi`,
          html: emailHTML,
          attachments,
        }),
      });
    ...
    <div className="flex-1 flex flex-row overflow-hidden" style={{ height: "100%", fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", background: "#F4F7FB" }}>
      ...
      <div style={{
        ...
        <div style={{ flexShrink: 0, padding: "16px", borderBottom: `1px solid ${sidebarBorderColor}`, background: sidebarHeaderBg }}>
          ...
          <div style={{ fontSize: "9px", color: "#64748B", marginTop: "2px" }}>Configura los datos de la propuesta comercial</div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: "16px", background: "#F4F7FB" }}>
          ...
          {/* Editor de Equipos */}
          <div style={floatingCard}>
            <div style={{ ...cardTitle, borderBottom: "none", paddingBottom: "2px", marginBottom: "8px" }}>Equipo Cotizado</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <div>
                ...
                <input value={eqTipo} onChange={e => setEqTipo(e.target.value)} placeholder="Ej. Ultrasonido"
              ...
              <div>
                ...
                {equipoMode === "catalogo" ? (
                  ...
                  <input value={eqModelo} onChange={e => setEqModelo(e.target.value)} placeholder="Ej. M9"
                    style={{ width: "100%", border: "1px solid #CBD5E1", borderRadius: "6px", padding: "5px 10px", fontSize: "11px" }} />
                )}
              </div>
            </div>
          ...
          {/* Características del equipo (Galería) */}
          <div style={floatingCard}>
            <div style={cardTitle}>Características (Galería)</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {features.map((feat, i) => (
                <div key={i}>
                  <label style={{ fontSize: "8.5px", color: "#64748B", fontWeight: 700 }}>PUNTO {i + 1}</label>
                  <input value={feat} onChange={e => setFeatures(p => p.map((x, j) => j === i ? e.target.value : x))} placeholder={`Característica ${i + 1}...`}
          ...
          <div style={floatingCard}>
            <div onClick={() => setCollItems(p => !p)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", userSelect: "none" }}>
              <div style={{ ...cardTitle, marginBottom: 0, borderBottom: "none", paddingBottom: 0 }}>Artículos / Conceptos</div>
                  ...
                  {items.map((item, i) => (
                    <div key={item.id} style={{ display: "flex", gap: "4px", background: "#F8FAFC", padding: "6px", borderRadius: "8px", border: "1px solid #E2E8F0" }}>
                      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "4px" }}>
                        <input value={item.descripcion} onChange={e => updateItem(item.id, "descripcion", e.target.value)} placeholder="Descripción del equipo..."
            ...
            {!collPoliticas && (
              <div style={{ marginTop: "12px", display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                <div>
                  ...
                  <input value={polGarantia} onChange={e => setPolGarantia(e.target.value)} placeholder="Ej. 12 meses"
                    style={{ width: "100%", border: "1px solid #CBD5E1", borderRadius: "6px", padding: "5px 8px", fontSize: "10.5px" }} />
                </div>
              </div>
            )}
            ...
            <div style={{ width: "816px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "35px" }}>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "12px" }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: "16px", fontWeight: 300, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "8px" }}>Propuesta Comercial</div>
                ...
                <div style={{ background: "#fff", border: "1px solid #CBD5E1", borderLeft: "4px solid #4E60A9", borderRadius: "12px", padding: "10px 14px", marginBottom: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                    <div style={{ fontSize: "10px", fontWeight: 800, color: "#4E60A9", textTransform: "uppercase", letterSpacing: "1px" }}>Equipo Cotizado</div>
                  </div>
                ...
                <div style={{ background: "#F8FAFC", border: "1px solid #CBD5E1", borderRadius: "12px", padding: "10px 14px", marginTop: "8px" }}>
                  <div style={{ fontSize: "10px", fontWeight: 800, color: "#4E60A9", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "8px", borderBottom: "2px solid #CBD5E1", paddingBottom: "4px" }}>
                    Galería del Equipo — {[eqMarca, eqModelo].filter(Boolean).join(" ") || eqTipo || "Producto"}
                  </div>
                ...
                <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: "12px", padding: "10px 14px", marginTop: "8px" }}>
                  <div style={{ fontSize: "10px", fontWeight: 800, color: "#4E60A9", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "8px", borderBottom: "2px solid #E2E8F0", paddingBottom: "4px" }}>Fotos del Equipo</div>
                  <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => uploadImage(b64 => setImgTransductor(b64))}
                      style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: "70px", height: "70px", border: "2px dashed #4E60A9", borderRadius: "8px", background: "transparent", cursor: "pointer", color: "#4E60A9", fontSize: "9px" }}>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderLeft: "4px solid #4E60A9", borderRadius: "12px", padding: "10px 14px", marginBottom: "12px" }}>
                    <div style={{ fontSize: "10px", fontWeight: 800, color: "#4E60A9", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "6px" }}>Propuesta Comercial</div>
                    <div style={{ fontSize: "11.5px", color: "#334155", lineHeight: "1.5" }}>
                      <F value={propuestaPDF} onChange={setPropuestaPDF} placeholder={generarParrafoAlcance() || "Descripción del alcance del suministro del equipo..."} multiline rows={4} />
                    </div>
                    ...
                    <div style={{ background: "#EEF0F7", border: "1px solid #C5CAE0", borderRadius: "10px", padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "12px" }}>
                      <div>
                        ...
                        <div style={{ fontSize: "11px", color: "#4E60A9" }}>Garantía y soporte técnico directo</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontSize: "10.5px" }}>
                    ...
                    <ul style={{ listStyle: "none", padding: 0 }}>
                      ...
                      <li style={{ position: "relative", paddingLeft: "12px", fontSize: "9.5px", color: "#475569", marginBottom: "3px" }}><strong>Tiempo de Entrega:</strong> <F value={polEntrega} onChange={setPolEntrega} placeholder={`Inmediata o 5 a 7 días hábiles.`} /></li>
                      <li style={{ position: "relative", paddingLeft: "12px", fontSize: "9.5px", color: "#475569", marginBottom: "3px" }}><strong>Garantía:</strong> <F value={polGarantia} onChange={setPolGarantia} placeholder="Garantía directa contra defectos de fábrica." /></li>
                    </ul>
                  </div>
                </div>
